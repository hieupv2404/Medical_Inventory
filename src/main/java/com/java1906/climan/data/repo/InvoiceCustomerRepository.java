...
//            ,nativeQuery = true)
        ...
        @Query(value="SELECT i.id, i.in_total, i.out_total, i.created_date, i.updated_date, s.customer_id, s.name, s.address, s.number_phone, s.email\n" +
                "  FROM invoice i, customer s\n" +
                "  WHERE i.type = 2 AND i.active_flag = 1 AND s.customer_id = i.customer_id;"
                ,nativeQuery = true)
        List<InvoiceCustomer> findInvoiceExport();
}
